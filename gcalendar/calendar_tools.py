...
async def create_event(
    ...
    # CRITICAL: Log initial attendees parameter value to investigate regression
    logger.debug(
        "[create_event] 🔍 FUNCTION ENTRY - Initial attendees parameter",
        extra={
            "attendees_value": attendees,
            "attendees_type": type(attendees).__name__
            if attendees is not None
            else "None",
            "attendees_repr": repr(attendees),
            "is_none": attendees is None,
            "is_string": isinstance(attendees, str),
            "is_list": isinstance(attendees, list),
        },
    )
    ...
    # If attendees value is a string, parse it to a list
    # Handles both comma-separated strings and JSON string arrays
    # CRITICAL: Add detailed logging to investigate regression (Test 75 vs Test 76+)
    logger.debug(
        "[create_event] 🔍 PARSING LOGIC ENTRY - Checking attendees parameter",
        extra={
            "attendees_value": attendees,
            "attendees_type": type(attendees).__name__,
            "is_truthy": bool(attendees),
            "is_string": isinstance(attendees, str),
            "condition_met": bool(attendees and isinstance(attendees, str)),
        },
    )

    if attendees and isinstance(attendees, str):
        logger.debug(
            "[create_event] 🔍 PARSING LOGIC ACTIVE - attendees is a string, starting parsing",
            extra={
                "attendees_value": attendees,
                "attendees_length": len(attendees),
                "attendees_repr": repr(attendees),
            },
        )
        # Try to parse as JSON array first (e.g., '["email1", "email2"]')
        try:
            parsed = json.loads(attendees)
            logger.debug(
                "[create_event] 🔍 JSON parsing succeeded",
                extra={
                    "parsed_value": parsed,
            ...
            if isinstance(parsed, list):
                attendees = [str(email).strip() for email in parsed if email]
                logger.info(
                    f"[create_event] Parsed attendees list from JSON string: {attendees}"
                )
            else:
                # If JSON parsing succeeded but result is not a list, treat as comma-separated
                logger.debug(
                    "[create_event] 🔍 JSON parsed but result is not a list, treating as comma-separated",
                    extra={
                        "parsed_value": parsed,
                        "parsed_type": type(parsed).__name__,
                    },
                ...
                logger.info(
                    f"[create_event] Parsed attendees list from comma-separated string: {attendees}"
                )
        except (json.JSONDecodeError, ValueError) as e:
            # Not valid JSON, treat as comma-separated string
            logger.debug(
                "[create_event] 🔍 JSON parsing failed, treating as comma-separated string",
        ...
        logger.debug(
            "[create_event] 🔍 PARSING LOGIC COMPLETE - Final attendees value",
            extra={
                "final_attendees": attendees,
                "final_type": type(attendees).__name__,
                "final_length": len(attendees)
        ...
        logger.debug(
            "[create_event] 🔍 PARSING LOGIC SKIPPED - Condition not met",
            extra={
                "attendees_value": attendees,
                "attendees_type": type(attendees).__name__,
                "is_truthy": bool(attendees),
                "is_string": isinstance(attendees, str),
                "reason": "attendees is not a string or is falsy",
            },
        )
    ...
    # CRITICAL: Add detailed logging to safeguard section to investigate regression
    logger.debug(
        "[create_event] 🔍 SAFEGUARD ENTRY - Checking attendees before building event_body",
        extra={
            "attendees_value": attendees,
            "attendees_type": type(attendees).__name__,
            "is_truthy": bool(attendees),
    ...
    if attendees:
        # CRITICAL: Ensure attendees is a list before iterating
        # If it's still a string (parsing didn't run or failed), convert it now
        logger.debug(
            "[create_event] 🔍 SAFEGUARD - attendees is truthy, checking if it's a string",
            extra={
                "attendees_value": attendees,
                "attendees_type": type(attendees).__name__,
                "is_string": isinstance(attendees, str),
        ...
        if isinstance(attendees, str):
            ...
            logger.warning(
                "[create_event] 🔍 SAFEGUARD TRIGGERED - Attendees was still a string, converting to list",
                extra={
                    "original_value": attendees,
                    "original_type": type(attendees).__name__,
                },
            )
            attendees = [a.strip() for a in attendees.split(",") if a.strip()]
            logger.warning(
                f"[create_event] Attendees was still a string, converted to list: {attendees}"
            )
        else:
            logger.debug(
                "[create_event] 🔍 SAFEGUARD - attendees is not a string, assuming it's already a list",
                extra={
                    "attendees_value": attendees,
                    "attendees_type": type(attendees).__name__,
                    "is_list": isinstance(attendees, list),
        ...
        # Now attendees is guaranteed to be a list
        logger.debug(
            "[create_event] 🔍 SAFEGUARD - Building event_body['attendees'] structure",
            extra={
                "attendees_list": attendees,
                "attendees_list_type": type(attendees).__name__,
                "attendees_list_length": len(attendees)
                if isinstance(attendees, list)
                else "N/A",
            },
        )
        event_body["attendees"] = [{"email": email} for email in attendees]
        # CRITICAL: Log the attendees structure to verify format (JSON format that will be sent to API)
        logger.debug(
            f"[create_event] Attendees structure for Google Calendar API (JSON format): {json.dumps(event_body['attendees'])}"
        )
        logger.debug(
            "[create_event] 🔍 SAFEGUARD COMPLETE - event_body['attendees'] built successfully",
            extra={
                "event_body_attendees": event_body["attendees"],
                "event_body_attendees_json": json.dumps(event_body["attendees"]),
            },
        )
    else:
        logger.debug(
            "[create_event] 🔍 SAFEGUARD SKIPPED - attendees is falsy, not adding to event_body",
            extra={
                "attendees_value": attendees,
                "attendees_type": type(attendees).__name__
                if attendees is not None
    ...
    if attachments:
        ...
        attendees_json = (
            json.dumps(event_body.get("attendees", []))
...
async def modify_event(
    ...
    # If attendees value is a string, parse it to a list
    # Handles both comma-separated strings and JSON string arrays
    if attendees is not None and isinstance(attendees, str):
        # Try to parse as JSON array first (e.g., '["email1", "email2"]')
        try:
            parsed = json.loads(attendees)
            if isinstance(parsed, list):
                attendees = [str(email).strip() for email in parsed if email]
                logger.info(
                    f"[modify_event] Parsed attendees list from JSON string: {attendees}"
                )
            else:
                # If JSON parsing succeeded but result is not a list, treat as comma-separated
                attendees = [a.strip() for a in attendees.split(",") if a.strip()]
                logger.info(
                    f"[modify_event] Parsed attendees list from comma-separated string: {attendees}"
                )
    ...
    if attendees is not None:
        ...
        if isinstance(attendees, str):
            ...
            attendees = [a.strip() for a in attendees.split(",") if a.strip()]
            logger.warning(
                f"[modify_event] Attendees was still a string, converted to list: {attendees}"
            )
        # Now attendees is guaranteed to be a list
        event_body["attendees"] = [{"email": email} for email in attendees]
        # CRITICAL: Log the attendees structure to verify format (JSON format that will be sent to API)
        logger.debug(
            f"[modify_event] Attendees structure for Google Calendar API (JSON format): {json.dumps(event_body['attendees'])}"
        )
